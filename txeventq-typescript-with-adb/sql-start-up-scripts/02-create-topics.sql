...
BEGIN
  ...
  DBMS_AQADM.set_queue_parameter('EVENT_TOPIC', 'SHARD_NUM', 4);
  -- Making the queue key-based to maintain order of events per key
  DBMS_AQADM.set_queue_parameter('EVENT_TOPIC', 'KEY_BASED_ENQUEUE', 1);
  -- Making consumers be sticky so the order is maintained during dequeue
  DBMS_AQADM.set_queue_parameter('EVENT_TOPIC', 'STICKY_DEQUEUE', 1);
  -- Start the queue
  DBMS_AQADM.start_queue(queue_name => 'EVENT_TOPIC');
END;
/
